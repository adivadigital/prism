...
await call('prism_new_document', { width: 1650, height: 1200, background: 'transparent' });

// Background
await call('prism_add_rectangle', { x: 0, y: 0, w: 1650, h: 1200, fill: '#0f1330', name: 'BG' });
...
// Duplicate the whole Poster group, then OFFSET the copy 820px to the right to sit beside the original
const dup = await call('prism_duplicate_layer', { id: gPoster });
await call('prism_offset_layer', { id: dup.id, dx: 820, dy: 0 });
console.log('duplicated + offset copy', dup.id, 'by +820,0');

const out = resolve(__dirname, 'poster-sidebyside.png');
